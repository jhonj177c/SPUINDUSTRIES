...
function initNavigation() {
    ...
    console.log('navPanelToggle encontrado:', !!navPanelToggle);
    console.log('navPanel encontrado:', !!navPanel);
    
    if (!navPanelToggle || !navPanel) {
    ...
    function togglePanel() {
        console.log('🔥 CLICK EN BOTÓN PANEL');
        
        // Toggle de clases
        navPanel.classList.toggle('active');
        navPanelToggle.classList.toggle('active');
    ...
    function closePanel() {
        ...
        navPanelToggle.classList.remove('active');
        document.body.style.overflow = '';
    }
    ...
    document.addEventListener('click', (e) => {
        if (navPanel.classList.contains('active') && 
            ...
            !navPanelToggle.contains(e.target)) {
            closePanel();
        }
    });
    ...
    // Cerrar panel al hacer clic en enlaces del panel
    const navPanelItems = navPanel.querySelectorAll('.nav-panel-item');
...
function initSmoothNavigation() {
    const navLinks = document.querySelectorAll('.nav-link, .nav-panel-item');
    ...
    navLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            if (navPanel && navPanel.classList.contains('active')) {
                navPanel.classList.remove('active');
                if (navPanelToggle) navPanelToggle.classList.remove('active');
                document.body.style.overflow = '';
            }
    ...
    if (form) {
        ...
        form.addEventListener('submit', (e) => {
            ...
            if (validateForm(data)) {
                ...
                showNotification('Por favor, completa todos los campos correctamente. Verifica que el nombre tenga al menos 2 caracteres, el email sea válido, el teléfono contenga solo números y el mensaje tenga al menos 3 caracteres.', 'error');
            }
        });
...
function validateForm(data) {
    ...
    if (!data.nombre || data.nombre.trim().length < 2) {
        console.log('Error: Nombre inválido - longitud:', data.nombre ? data.nombre.trim().length : 0);
        return false;
    }
    
    // Validar email
    if (!data.email || !emailRegex.test(data.email)) {
        console.log('Error: Email inválido - valor:', data.email);
        return false;
    }
    
    // Validar teléfono
    if (!data.telefono || !phoneRegex.test(data.telefono)) {
        console.log('Error: Teléfono inválido - valor:', data.telefono);
        return false;
    }
    
    // Validar mensaje
    if (!data.mensaje || data.mensaje.trim().length < 3) {
        console.log('Error: Mensaje inválido - longitud:', data.mensaje ? data.mensaje.trim().length : 0, 'valor:', data.mensaje);
        return false;
    }
